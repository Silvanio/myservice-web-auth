    ...
    <div class="p-grid">
        <div class="p-col-12">
            <div class="card">
                <h1>
                    {{ title }}
                    <Button style="float: right" icon="pi pi-plus-circle" class="p-button-rounded" @click="openCreate" v-tooltip.left="$t('user.lbl_create_title')"/>
                </h1>
                ...
                <div class="p-fluid p-formgrid p-grid">
                    <div class="p-field p-col-12">
                        <DataTable class="p-datatable-responsive" :value="page.content" :lazy="true" :filters="pageable.entity" :paginator="true" :rows="10"
                            ...
                            <Column field="name" :header="$t('user.lbl_name')" :sortable="true" filterMatchMode="contains">
                                <template #filter>
                                    <InputText type="text" @blur="onPage()" v-model="pageable.entity['name']" class="p-column-filter" :placeholder="$t('user.lbl_name')"/>
                                </template>
                            </Column>

                            <Column field="username" :header="$t('user.lbl_username')" :sortable="true" filterMatchMode="contains">
                                <template #filter>
                                    <InputText type="text" @blur="onPage()" v-model="pageable.entity['username']" class="p-column-filter"
                                               :placeholder="$t('user.lbl_username')"/>
                                </template>
                            </Column>

                            <Column field="email" :header="$t('user.lbl_email')" :sortable="true" filterMatchMode="contains">
                                <template #filter>
                                    <InputText type="text" @blur="onPage()" v-model="pageable.entity['email']" class="p-column-filter" :placeholder="$t('user.lbl_email')"/>
                                </template>
                            </Column>

                            <Column field="company.name" :header="$t('menu.lbl_company')" :sortable="true" filterMatchMode="contains">
                                <template #filter>
                                    <InputText type="text" @blur="onPage()" v-model="pageable.entity.company['name']" class="p-column-filter"
